// app/api/_sites-diagnose/route.ts
import { NextResponse } from 'next/server';
import { headers } from 'next/headers';
import { supabaseAdmin } from '@/lib/supabase/admin';
import { getServerSupabase } from '@/lib/supabase/server';

const SELECT = 'id, slug, template_name, domain_lc, published, is_site, archived';
export async function GET() {
  ...
  const ssr = await s1.from('templates').select(SELECT)
    .eq('is_site', true).eq('published', true).eq('archived', false)
    .in('domain_lc', variants).maybeSingle();

  const admin = await supabaseAdmin.from('templates').select(SELECT)
    .eq('is_site', true).eq('published', true).eq('archived', false)
    .in('domain_lc', variants).maybeSingle();
