...
export async function PATCH(req: NextRequest, { params }:{ params:{ id:string } }) {
  ...
  const { data: doc, error: e0 } = await svc
    ...
    .select('id, merchant_id, requirement_id, status')
    .eq('id', params.id).maybeSingle();
  if (e0 || !doc) return NextResponse.json({ error:'Not found' }, { status:404 });

  if (action === 'approve') {
    const { error: e1 } = await svc.from('compliance_docs').update({
      status:'approved', reviewer: user.email || user.id, reviewed_at: new Date().toISOString(),
      expires_at, issued_at
    }).eq('id', params.id);
    if (e1) return NextResponse.json({ error: e1.message }, { status:500 });
  } else if (action === 'reject') {
    const { error: e2 } = await svc.from('compliance_docs').update({
      status:'rejected', reviewer: user.email || user.id, reviewed_at: new Date().toISOString(),
  ...
  .from('compliance_docs')
  .select('id, merchant_id, requirement_id, kind, expires_at, issued_at, compliance_requirements(code, operation_type, juris_state), merchants(user_id, display_name, name), profiles:merchant_compliance_profiles(state, county, operation_type)')
  .eq('id', params.id).maybeSingle();

    // find chef owner email
    const { data: owner } = await svc.auth.admin.getUserById(detail?.merchants[0]?.user_id || '');
    const toEmail = owner?.user?.email || null;

    const prettyReq = `${detail?.compliance_requirements[0]?.juris_state}/${detail?.compliance_requirements[0]?.operation_type}/${detail?.compliance_requirements[0]?.code}`;
    ...
    await svc.from('email_outbox').insert({
        to_user_id: detail?.merchants[0]?.user_id,
        to_email: toEmail,
    ...
    await svc.from('email_outbox').insert({
        ...
        html: `<p>Hi ${title},</p>
            ...
            <p>Please re-upload a correct document here: <a href="${process.env.APP_BASE_URL}/chef/compliance?focus=${detail?.compliance_requirements[0]?.code}">Open compliance</a></p>`
    });
    }

    // Optional: Slack for ops (set COMPLIANCE_SLACK_WEBHOOK_URL)
