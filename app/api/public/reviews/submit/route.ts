...
export async function GET(req: NextRequest) {
  ...
  let sel = db.from("reviews")
    ...
    .order(sort === "top" ? "rating" : "created_at", { ascending: false });
  ...
  if (!includeSummary) return NextResponse.json({ reviews: data || [], hasMore: (data||[]).length === limit });
  ...
  let total = 0, n = 0;
  (allRatings || []).forEach(r => { const v = (r as any).rating as number; if (v>=1 && v<=5) { stars[String(v)]++; total+=v; n++; } });
  const avg = n ? total / n : 0;

  return NextResponse.json({ reviews: data || [], hasMore: (data||[]).length === limit, summary: { count: n, avg, stars } });
}
