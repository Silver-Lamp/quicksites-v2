...
export default async function GigsIndexPage() {
  ...
  return (
    <div className="mx-auto min-h-screen max-w-2xl px-4 py-12 text-zinc-900">
      <div className="mb-2 text-sm font-medium text-sky-600">🧺 Store-walk gigs</div>
      ...
      {gigs.length === 0 ? (
        <p className="mt-10 text-zinc-500">No open gigs right now. Check back soon.</p>
      ) : (
        <ul className="mt-8 space-y-2">
          {gigs.map((g) => (
            <li key={g.id}>
              <Link
                href={`/gigs/${g.id}`}
                className="flex items-center justify-between gap-3 rounded-xl border border-zinc-200 bg-white p-4 transition hover:border-sky-300 hover:shadow-sm"
              >
                <div className="min-w-0">
                  <div className="truncate font-semibold text-zinc-900">{g.store_name}</div>
                  <div className="truncate text-xs text-zinc-500">{gigWhere(g)}</div>
                </div>
      ...
      <p className="mt-10 text-xs text-zinc-400">
        ...
        <Link href="/walker" className="text-sky-600 hover:underline">
          walk board
        </Link>
        .
      </p>
