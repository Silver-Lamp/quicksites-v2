  ...
  import PreviewBridge from '@/app/preview/preview-bridge'; // NEW
  ...
  async function manualById(
    ...
    // backstop: if composed data lacks pages, fall back to canonical pages
    const parse = (x: any) => (typeof x === 'string' ? (() => { try { return JSON.parse(x); } catch { return null; } })() : x);
    const hasPages = (d: any) => !!d && Array.isArray(d.pages) && d.pages.length > 0;
    if (!hasPages(site?.data)) {
      const cData = parse(c?.data) ?? {};
  ...
  async function loadSiteForRequest(opts: {
    ...
    const { previewVersionId, explicitTemplateId, explicitSlug } = opts;
  ...
  export async function generateMetadata({
    ...
    // NOTE: Next 15 — searchParams is async
    ...
    const payload = await loadSiteForRequest({ previewVersionId, explicitTemplateId, explicitSlug });
    if (!payload) return {};
    const { site } = payload;
  
    // pick slug from path first, then ?page=, then first page
