...
//
// Dark-mode by default (standalone utility page). Pure math (haversine miles) + the Maps
// URL builder + the optimizer are imported from lib/route/* so both a manual reorder AND
// the "Show example" demo run entirely CLIENT-SIDE — the example uses coordinates (no
// geocoding, no server round-trip), so it never consumes the endpoint's per-IP rate limit.
...
function RouteMap({ start, ordered }: { start: Ordered; ordered: Ordered[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="h-auto w-full rounded-xl border border-zinc-800 bg-zinc-950/60" role="img" aria-label="Route map preview">
      <polyline points={path} fill="none" stroke="#38bdf8" strokeWidth={2} strokeDasharray="4 4" strokeLinejoin="round" opacity={0.8} />
      {/* start marker */}
      <circle cx={x(start.longitude)} cy={y(start.latitude)} r={9} fill="#e4e4e7" />
      <text x={x(start.longitude)} y={y(start.latitude) + 3} textAnchor="middle" fontSize={9} fill="#09090b" fontWeight="700">◎</text>
      {ordered.map((p, i) => {
        const isReturn = i === ordered.length - 1 && p.latitude === start.latitude && p.longitude === start.longitude;
        if (isReturn) return null;
        return (
          <g key={i}>
            <circle cx={x(p.longitude)} cy={y(p.latitude)} r={9} fill="#0ea5e9" />
...
export default function RoutePlanner({ initialStops, initialStart }: { initialStops: string[]; initialStart: string }) {
  ...
  const inputCls = 'w-full rounded-lg border border-zinc-700 bg-zinc-950/60 px-3 py-2.5 text-[15px] text-zinc-100 placeholder-zinc-500 outline-none transition focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20';

  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-zinc-950 to-black text-zinc-100">
      <div className="mx-auto max-w-2xl px-4 pb-28 pt-8 sm:pt-12">
        <header>
          <div className="flex items-start justify-between gap-3">
            <div>
              <h1 className="text-2xl font-bold tracking-tight text-white sm:text-3xl">🗺️ Route planner</h1>
            ...
            <button
              type="button"
              onClick={showExample}
              className="shrink-0 rounded-lg border border-sky-500/40 bg-sky-500/10 px-3 py-2 text-xs font-semibold text-sky-300 transition hover:border-sky-400 hover:bg-sky-500/20"
            >
              ✨ Show example
            </button>
          </div>
        ...
        {/* ── Stops card ── */}
        <section className="mt-6 rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4 shadow-sm sm:p-5">
          <label className="block">
            <span className="text-xs font-semibold uppercase tracking-wide text-zinc-400">Start point</span>
            <span className="ml-1 text-xs font-normal text-zinc-500">optional — your first stop if blank</span>
          ...
          <div className="mt-5 flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wide text-zinc-400">
              Stops {filledCount > 0 && <span className="ml-1 rounded-full bg-sky-500/20 px-1.5 py-0.5 text-[11px] font-bold text-sky-300">{filledCount}</span>}
            </span>
            <button type="button" onClick={() => setShowPaste((v) => !v)} className="text-xs font-semibold text-sky-400 hover:text-sky-300">
              {showPaste ? 'Close paste' : '＋ Paste a list'}
            </button>
          </div>

          {showPaste && (
            <div className="mt-2 rounded-lg border border-dashed border-zinc-700 bg-zinc-950/50 p-2.5">
              <textarea value={pasteText} onChange={(e) => setPasteText(e.target.value)} rows={4}
                placeholder={'One stop per line:\nStore A, 500 Oak Ave, Springfield, IL\nStore B, 88 Elm St, Springfield, IL'}
                className="w-full resize-y rounded-md border border-zinc-700 bg-zinc-950/60 px-2.5 py-2 font-mono text-xs text-zinc-100 placeholder-zinc-600 outline-none focus:border-sky-500" />
              <div className="mt-2 flex justify-end gap-2">
                <button type="button" onClick={() => { setPasteText(''); setShowPaste(false); }} className="rounded-md px-3 py-1.5 text-xs font-semibold text-zinc-400 hover:text-zinc-200">Cancel</button>
                <button type="button" onClick={applyPaste} className="rounded-md bg-zinc-100 px-3 py-1.5 text-xs font-semibold text-zinc-900 hover:bg-white">Add lines</button>
              </div>
            </div>
          )}
          ...
          <ul className="mt-2 space-y-2">
            {rows.map((r, i) => (
              <li key={r.id} className="flex items-center gap-2">
                <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-zinc-800 text-xs font-bold text-zinc-400 tabular-nums">{i + 1}</span>
                <input
                  value={r.value}
                ...
                <button type="button" onClick={() => removeRow(r.id)} aria-label={`Remove stop ${i + 1}`}
                  className="grid h-9 w-9 shrink-0 place-items-center rounded-lg text-zinc-500 transition hover:bg-red-500/10 hover:text-red-400">✕</button>
              </li>
            ))}
          </ul>

          <button type="button" onClick={addRow} className="mt-3 w-full rounded-lg border border-dashed border-zinc-700 py-2 text-sm font-semibold text-zinc-400 transition hover:border-sky-500 hover:text-sky-300">
            ＋ Add stop
          </button>

          <label className="mt-4 flex items-center gap-2.5 text-sm text-zinc-300">
            <input type="checkbox" checked={roundTrip} onChange={(e) => setRoundTrip(e.target.checked)} className="h-4 w-4 rounded accent-sky-500" />
            Return to start at the end (round trip)
          </label>

          {error && <div className="mt-3 rounded-lg bg-red-500/10 px-3 py-2 text-sm font-medium text-red-300">{error}</div>}
        </section>

        {/* ── Result ── */}
        {route && derived && (
          <section ref={resultRef} className="mt-6 scroll-mt-4 rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4 shadow-sm sm:p-5">
            <div className="flex items-baseline justify-between">
              <h2 className="text-lg font-bold text-white">Your route</h2>
              <span className="text-sm font-semibold tabular-nums text-zinc-400">≈ {derived.miles} mi</span>
            </div>
            ...
            <ol className="mt-4 space-y-1.5">
              <li className="flex items-center gap-3 rounded-lg bg-zinc-800/50 px-2.5 py-2">
                <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-zinc-100 text-xs font-bold text-zinc-900">◎</span>
                <span className="min-w-0 flex-1 truncate text-sm font-medium text-zinc-100">{route.start.label}</span>
              ...
              {route.ordered.map((r, i) => {
                ...
                return (
                  <li
                    ...
                    className={`flex items-center gap-2.5 rounded-lg px-2.5 py-2 transition ${dragIndex === i ? 'opacity-40' : ''} ${isReturn ? 'bg-zinc-800/50' : 'bg-transparent hover:bg-sky-500/10'}`}
                  >
                    <span className={`grid h-7 w-7 shrink-0 place-items-center rounded-full text-xs font-bold ${isReturn ? 'bg-zinc-700 text-zinc-400' : 'bg-sky-500 text-white'}`}>
                      {isReturn ? '◎' : i + 1}
                    </span>
                    <span className="min-w-0 flex-1 truncate text-sm text-zinc-200">{r.label}</span>
                    {isReturn ? (
                      <span className="text-[11px] font-semibold uppercase tracking-wide text-zinc-500">Return</span>
                    ) : (
                      <div className="flex shrink-0 items-center gap-0.5">
                        <button type="button" onClick={() => move(i, i - 1)} disabled={i === 0} aria-label="Move up"
                          className="grid h-7 w-7 place-items-center rounded text-zinc-500 hover:bg-zinc-800 hover:text-zinc-200 disabled:opacity-30">↑</button>
                        ...
                        <a href={buildSingleStopUrl(r)} target="_blank" rel="noopener noreferrer" aria-label="Navigate to this stop"
                          className="hidden h-7 w-7 place-items-center rounded text-zinc-500 hover:bg-zinc-800 hover:text-sky-400 sm:grid" title="Navigate to just this stop">➤</a>
                        <span className="hidden cursor-grab select-none px-1 text-zinc-600 sm:inline" title="Drag to reorder">⠿</span>
                      </div>
                    )}
                  </li>
                );
            ...
            <div className="mt-4 flex flex-col gap-2 sm:flex-row">
              ...
              <button type="button" onClick={copyMaps}
                className="flex items-center justify-center gap-2 rounded-xl border border-zinc-700 px-4 py-3 text-sm font-semibold text-zinc-200 transition hover:bg-zinc-800">
            ...
            <p className="mt-3 text-[11px] leading-relaxed text-zinc-500">{route.note}</p>
          </section>
        )}
      </div>

      {/* ── Sticky action bar (mobile-first) ── */}
      <div className="fixed inset-x-0 bottom-0 border-t border-zinc-800 bg-zinc-950/90 px-4 py-3 backdrop-blur">
