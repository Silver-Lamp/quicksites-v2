...
//
// Theme: dark by DEFAULT, with a header toggle (persisted to localStorage). The page is
// self-contained — every color comes from an explicit `t(light, dark)` ternary keyed on
// local state, NOT Tailwind `dark:` variants. That matters because the surrounding app
// applies a global `.dark`/`[data-theme=dark]` (QuickSites defaults dark), so `dark:`
// utilities would be pinned on regardless of a local toggle. Concrete classes dodge that.
//
...
// SVG fills are attributes (not classes), so the palette is picked from the `dark` flag.
function RouteMap({ start, ordered, dark }: { start: Ordered; ordered: Ordered[]; dark: boolean }) {
  ...
  return (
    <svg
      viewBox={`0 0 ${W} ${H}`}
      className={`h-auto w-full rounded-xl border ${dark ? 'border-zinc-800 bg-zinc-950/60' : 'border-zinc-200 bg-zinc-50'}`}
      role="img"
...
export default function RoutePlanner({ initialStops, initialStart }: { initialStops: string[]; initialStart: string }) {
  ...
  // Theme picker: return the light OR dark class string for this element.
  ...
  const inputCls = `w-full rounded-lg border px-3 py-2.5 text-[15px] outline-none transition focus:border-sky-500 focus:ring-2 ${t(
    'border-zinc-300 bg-white text-zinc-900 placeholder-zinc-400 focus:ring-sky-100',
    'border-zinc-700 bg-zinc-950/60 text-zinc-100 placeholder-zinc-500 focus:ring-sky-500/20',
  )}`;

  return (
    <div className={`min-h-screen bg-gradient-to-b ${t('from-sky-50 via-white to-white text-zinc-900', 'from-slate-950 via-zinc-950 to-black text-zinc-100')}`}>
      <div className="mx-auto max-w-2xl px-4 pb-28 pt-8 sm:pt-12">
        <header>
          <div className="flex items-start justify-between gap-3">
            <div>
              <h1 className={`text-2xl font-bold tracking-tight sm:text-3xl ${t('text-zinc-900', 'text-white')}`}>🗺️ Route planner</h1>
              <p className={`mt-1.5 text-sm leading-relaxed ${t('text-zinc-500', 'text-zinc-400')}`}>
            ...
            <div className="flex shrink-0 items-center gap-2">
              <button
                ...
                className={`grid h-9 w-9 place-items-center rounded-lg border transition ${t('border-zinc-300 text-zinc-600 hover:bg-zinc-100', 'border-zinc-700 text-zinc-300 hover:bg-zinc-800')}`}
              >
                {isDark ? '☀️' : '🌙'}
              </button>
              <button
                type="button"
                onClick={showExample}
                className={`rounded-lg border px-3 py-2 text-xs font-semibold transition ${t('border-sky-300 bg-sky-50 text-sky-700 hover:border-sky-400 hover:bg-sky-100', 'border-sky-500/40 bg-sky-500/10 text-sky-300 hover:border-sky-400 hover:bg-sky-500/20')}`}
              >
                ✨ Show example
              </button>
            </div>
        ...
        {/* ── Stops card ── */}
        <section className={`mt-6 rounded-2xl border p-4 shadow-sm sm:p-5 ${t('border-zinc-200 bg-white', 'border-zinc-800 bg-zinc-900/60')}`}>
          <label className="block">
            <span className={`text-xs font-semibold uppercase tracking-wide ${t('text-zinc-500', 'text-zinc-400')}`}>Start point</span>
            <span className={`ml-1 text-xs font-normal ${t('text-zinc-400', 'text-zinc-500')}`}>optional — your first stop if blank</span>
          ...
          <div className="mt-5 flex items-center justify-between">
            <span className={`text-xs font-semibold uppercase tracking-wide ${t('text-zinc-500', 'text-zinc-400')}`}>
              Stops {filledCount > 0 && <span className={`ml-1 rounded-full px-1.5 py-0.5 text-[11px] font-bold ${t('bg-sky-100 text-sky-700', 'bg-sky-500/20 text-sky-300')}`}>{filledCount}</span>}
            </span>
            <button type="button" onClick={() => setShowPaste((v) => !v)} className={`text-xs font-semibold ${t('text-sky-600 hover:text-sky-700', 'text-sky-400 hover:text-sky-300')}`}>
              {showPaste ? 'Close paste' : '＋ Paste a list'}
            </button>
          </div>

          {showPaste && (
            <div className={`mt-2 rounded-lg border border-dashed p-2.5 ${t('border-zinc-300 bg-zinc-50', 'border-zinc-700 bg-zinc-950/50')}`}>
              <textarea value={pasteText} onChange={(e) => setPasteText(e.target.value)} rows={4}
                placeholder={'One stop per line:\nStore A, 500 Oak Ave, Springfield, IL\nStore B, 88 Elm St, Springfield, IL'}
                className={`w-full resize-y rounded-md border px-2.5 py-2 font-mono text-xs outline-none focus:border-sky-500 ${t('border-zinc-300 bg-white text-zinc-900 placeholder-zinc-400', 'border-zinc-700 bg-zinc-950/60 text-zinc-100 placeholder-zinc-600')}`} />
              <div className="mt-2 flex justify-end gap-2">
                <button type="button" onClick={() => { setPasteText(''); setShowPaste(false); }} className={`rounded-md px-3 py-1.5 text-xs font-semibold ${t('text-zinc-500 hover:text-zinc-700', 'text-zinc-400 hover:text-zinc-200')}`}>Cancel</button>
                <button type="button" onClick={applyPaste} className={`rounded-md px-3 py-1.5 text-xs font-semibold ${t('bg-zinc-900 text-white hover:bg-zinc-700', 'bg-zinc-100 text-zinc-900 hover:bg-white')}`}>Add lines</button>
              </div>
            </div>
          )}
          ...
          <ul className="mt-2 space-y-2">
            {rows.map((r, i) => (
              <li key={r.id} className="flex items-center gap-2">
                <span className={`grid h-7 w-7 shrink-0 place-items-center rounded-full text-xs font-bold tabular-nums ${t('bg-zinc-100 text-zinc-500', 'bg-zinc-800 text-zinc-400')}`}>{i + 1}</span>
                <input
                  value={r.value}
                ...
                <button type="button" onClick={() => removeRow(r.id)} aria-label={`Remove stop ${i + 1}`}
                  className={`grid h-9 w-9 shrink-0 place-items-center rounded-lg transition ${t('text-zinc-400 hover:bg-red-50 hover:text-red-500', 'text-zinc-500 hover:bg-red-500/10 hover:text-red-400')}`}>✕</button>
              </li>
            ))}
          </ul>

          <button type="button" onClick={addRow} className={`mt-3 w-full rounded-lg border border-dashed py-2 text-sm font-semibold transition ${t('border-zinc-300 text-zinc-500 hover:border-sky-400 hover:text-sky-600', 'border-zinc-700 text-zinc-400 hover:border-sky-500 hover:text-sky-300')}`}>
            ＋ Add stop
          </button>

          <label className={`mt-4 flex items-center gap-2.5 text-sm ${t('text-zinc-700', 'text-zinc-300')}`}>
            <input type="checkbox" checked={roundTrip} onChange={(e) => setRoundTrip(e.target.checked)} className={`h-4 w-4 rounded ${t('accent-sky-600', 'accent-sky-500')}`} />
            Return to start at the end (round trip)
          </label>

          {error && <div className={`mt-3 rounded-lg px-3 py-2 text-sm font-medium ${t('bg-red-50 text-red-600', 'bg-red-500/10 text-red-300')}`}>{error}</div>}
        </section>

        {/* ── Result ── */}
        {route && derived && (
          <section ref={resultRef} className={`mt-6 scroll-mt-4 rounded-2xl border p-4 shadow-sm sm:p-5 ${t('border-zinc-200 bg-white', 'border-zinc-800 bg-zinc-900/60')}`}>
            <div className="flex items-baseline justify-between">
              <h2 className={`text-lg font-bold ${t('text-zinc-900', 'text-white')}`}>Your route</h2>
              <span className={`text-sm font-semibold tabular-nums ${t('text-zinc-500', 'text-zinc-400')}`}>≈ {derived.miles} mi</span>
            </div>
            ...
            <ol className="mt-4 space-y-1.5">
              <li className={`flex items-center gap-3 rounded-lg px-2.5 py-2 ${t('bg-zinc-50', 'bg-zinc-800/50')}`}>
                <span className={`grid h-7 w-7 shrink-0 place-items-center rounded-full text-xs font-bold ${t('bg-zinc-900 text-white', 'bg-zinc-100 text-zinc-900')}`}>◎</span>
                <span className={`min-w-0 flex-1 truncate text-sm font-medium ${t('text-zinc-900', 'text-zinc-100')}`}>{route.start.label}</span>
                <span className={`text-[11px] font-semibold uppercase tracking-wide ${t('text-zinc-400', 'text-zinc-500')}`}>Start</span>
              </li>
              {route.ordered.map((r, i) => {
                ...
                return (
                  <li
                    ...
                    className={`flex items-center gap-2.5 rounded-lg px-2.5 py-2 transition ${dragIndex === i ? 'opacity-40' : ''} ${isReturn ? t('bg-zinc-50', 'bg-zinc-800/50') : t('bg-white hover:bg-sky-50/60', 'bg-transparent hover:bg-sky-500/10')}`}
                  >
                    <span className={`grid h-7 w-7 shrink-0 place-items-center rounded-full text-xs font-bold ${isReturn ? t('bg-zinc-200 text-zinc-500', 'bg-zinc-700 text-zinc-400') : 'bg-sky-600 text-white'}`}>
                      {isReturn ? '◎' : i + 1}
                    </span>
                    <span className={`min-w-0 flex-1 truncate text-sm ${t('text-zinc-800', 'text-zinc-200')}`}>{r.label}</span>
                    {isReturn ? (
                      <span className={`text-[11px] font-semibold uppercase tracking-wide ${t('text-zinc-400', 'text-zinc-500')}`}>Return</span>
                    ) : (
                      <div className="flex shrink-0 items-center gap-0.5">
                        <button type="button" onClick={() => move(i, i - 1)} disabled={i === 0} aria-label="Move up"
                          className={`grid h-7 w-7 place-items-center rounded disabled:opacity-30 ${t('text-zinc-400 hover:bg-zinc-100 hover:text-zinc-700', 'text-zinc-500 hover:bg-zinc-800 hover:text-zinc-200')}`}>↑</button>
                        ...
                        <a href={buildSingleStopUrl(r)} target="_blank" rel="noopener noreferrer" aria-label="Navigate to this stop"
                          className={`hidden h-7 w-7 place-items-center rounded sm:grid ${t('text-zinc-400 hover:bg-zinc-100 hover:text-sky-600', 'text-zinc-500 hover:bg-zinc-800 hover:text-sky-400')}`} title="Navigate to just this stop">➤</a>
                        <span className={`hidden cursor-grab select-none px-1 sm:inline ${t('text-zinc-300', 'text-zinc-600')}`} title="Drag to reorder">⠿</span>
                      </div>
                    )}
                  </li>
                );
            ...
            <div className="mt-4 flex flex-col gap-2 sm:flex-row">
              <a href={derived.mapsUrl} target="_blank" rel="noopener noreferrer"
                className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-emerald-500">
                Open in Google Maps →
              </a>
              <button type="button" onClick={copyMaps}
                className={`flex items-center justify-center gap-2 rounded-xl border px-4 py-3 text-sm font-semibold transition ${t('border-zinc-300 text-zinc-700 hover:bg-zinc-50', 'border-zinc-700 text-zinc-200 hover:bg-zinc-800')}`}>
                {copied ? '✓ Copied' : 'Copy link'}
              </button>
            </div>

            {route.unresolved.length > 0 && (
              <p className={`mt-3 rounded-lg px-3 py-2 text-xs ${t('bg-amber-50 text-amber-700', 'bg-amber-500/10 text-amber-300')}`}>
                Couldn’t locate: {route.unresolved.join(', ')} — check the spelling or add a city/ZIP.
              </p>
            )}
            <p className={`mt-3 text-[11px] leading-relaxed ${t('text-zinc-400', 'text-zinc-500')}`}>{route.note}</p>
          </section>
        )}
      </div>

      {/* ── Sticky action bar (mobile-first) ── */}
      <div className={`fixed inset-x-0 bottom-0 border-t px-4 py-3 backdrop-blur ${t('border-zinc-200 bg-white/90', 'border-zinc-800 bg-zinc-950/90')}`}>
        <div className="mx-auto max-w-2xl">
          <button type="button" onClick={optimize} disabled={busy || filledCount === 0}
            className="w-full rounded-xl bg-sky-600 px-4 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-sky-500 disabled:opacity-40">
