...
const InspirationalQuote = dynamic(() => import('@/components/ui/inspirational-quote'), {
  ssr: false,
});
...
export default function AppHeader() {
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('scroll', wake, pc);
    window.addEventListener('wheel', wake, pc);
    window.addEventListener('touchmove', wake, pc);
    ...
    return () => {
      if (t) window.clearTimeout(t);
      window.removeEventListener('scroll', wake, pc as any);
      window.removeEventListener('wheel', wake, pc as any);
      window.removeEventListener('touchmove', wake, pc as any);
  ...
  React.useEffect(() => {
    ...
    console.debug('[🧭 AppHeader Role Info]', {
      ...
      traceId,
      sessionId,
      displayFrom: displayAuth.source,
      displayEmail: displayAuth.email,
      displayRole: displayAuth.role,
    });
  }, [
    user?.email,
    role,
  ...
  return (
    <header
      ...
      className={clsx(
        ...
        guest
          ? 'bg-gray-900 text-zinc-300 border-zinc-800'
    ...
    >
      ...
      {!hydrated ? (
        <div className="max-w-screen-lg mx-auto flex justify-between items-center">
          ...
          <SignInLink variant="inline" className="pointer-events-auto" />
        </div>
      ) : guest ? (
        <div className="max-w-screen-lg mx-auto flex justify-between items-center">
          <div className="flex items-center gap-4">
          ...
          >
            ...
            <div className="leading-tight">
              ...
              <div className="text-zinc-400 text-xs">
                role: {displayAuth.role ?? (role as string) ?? 'user'}
              </div>
            </div>
          </div>
        </div>
