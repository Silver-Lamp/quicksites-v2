...
export default function Tooltip({ content, children, clickable = false, placement = 'top', labelledById = `tooltip-${Math.random().toString(36).slice(2, 10)}` }: TooltipProps) {
  const ref = useRef<HTMLDivElement>(null);
const [autoPlacement, setAutoPlacement] = useState<'top' | 'bottom' | 'left' | 'right'>(placement);
  const [prevPlacement, setPrevPlacement] = useState<'top' | 'bottom' | 'left' | 'right'>(placement);
...
useEffect(() => {
  ...
  else if (placement === 'bottom' && rect.bottom > window.innerHeight - buffer) setAutoPlacement('top');
  else if (placement === 'left' && rect.left < buffer) setAutoPlacement('right');
  else if (placement === 'right' && rect.right > window.innerWidth - buffer) setAutoPlacement('left');
}, [placement]);
