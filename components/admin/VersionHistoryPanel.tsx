...
export default function VersionHistoryPanel({ slug, onRestore }: {
    ...
    const fetchVersions = async () => {
      const [versionRes, currentRes] = await Promise.all([
        supabase
          ...
          .select(`id, created_at, snapshot_id, label, slug, branding_profile_id, branding_profiles ( id, name, primary_color, secondary_color, font_family )`)
          .eq('slug', slug)
  ...
  if (versions.length === 0) return <p className="text-sm text-gray-500">No version history available.</p>;
      ...
      <ul className="space-y-2">
        {versions.map(v => {
          ...
          return (
            ...
            >
              <div>
                <div className="text-sm text-gray-100">Snapshot: <code>{v.snapshot_id}</code></div>
                <div className="text-xs text-gray-400">Label: {v.label || 'No label'}</div>
                <div className="text-xs text-gray-500">Published: {new Date(v.created_at).toLocaleString()}</div>
                <div className="text-xs mt-1" style={{ fontFamily: v.branding_profiles?.font_family || 'sans-serif' }}>
              ...
              <div className="flex gap-2">
                ...
                {confirmingDelete === v.id ? (
                  <Button size="sm" variant="destructive" onClick={async () => {
                    const { error } = await supabase.from('published_versions').delete().eq('id', v.id);
