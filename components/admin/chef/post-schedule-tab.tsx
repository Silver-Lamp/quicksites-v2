...
type Meal = { id:string; title:string; slug?:string|null; price_cents:number; auto_last_call?: boolean|null; };
type Sched = { id:string; network:string; kind:string; scheduled_for:string; status:string; text:string; link_url:string; };
...
export default function PostScheduleTab({ siteId }: { siteId: string }) {
  ...
  const [netX, setNetX] = useState(true);
  const [caption, setCaption] = useState('');
  const [autoLastCall, setAutoLastCall] = useState(true);
  ...
  async function loadMeals() {
    ...
    const d = await r.json();
    const arr:Meal[] = (d?.meals ?? []).map((m:any)=>({ id:m.id, title:m.title, slug:m.slug, price_cents:m.price_cents, auto_last_call:m.auto_last_call }));
    setMeals(arr);
  }
  async function loadSchedules() {
    const r = await fetch('/api/chef/schedule/list');
    const d = await r.json();
    setSchedules(d?.posts ?? []);
  }

  useEffect(() => { loadMeals(); loadSchedules(); }, [siteId]);
  useEffect(() => {
    const m = meals.find(x => x.id === mealId) || null;
    setSelectedMeal(m);
    if (m) {
      setCaption(`🍽️ ${m.title} is LIVE on delivered.menu — $${(m.price_cents/100).toFixed(2)}. Limited portions.`);
    ...
    <div className="space-y-4">
      <div className="rounded-2xl border p-4 space-y-3">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          ...
          <div className="mt-2">
            <div className="text-xs text-muted-foreground mb-1">Webhooks</div>
            {hooks.length === 0 ? (
                ...
                {hooks.map(h => {
                    ...
                    <label key={h.id} className={[
                        ...
                        checked={on}
                        onChange={(e) => setSelectedHooks(on ? selectedHooks.filter(x=>x!==h.id) : [...selectedHooks, h.id])}
                        />
                        {h.name} ({h.kind})
                    </label>
                    );
        ...
        <div>
          <Label>Caption</Label>
          <textarea className="w-full rounded-md border px-3 py-2 text-sm" rows={4}
                    value={caption} onChange={(e)=>setCaption(e.target.value)} />
        </div>

        <div className="flex items-center gap-3">
          <label className="flex items-center gap-2 text-sm">
            <input type="checkbox" checked={autoLastCall} onChange={(e)=>setAutoLastCall(e.target.checked)} />
            Auto last-call when ≤ 3
          </label>
          {selectedMeal && (
            <ShareMenu url={pageUrl} title={selectedMeal.title} />
          )}
        </div>
        ...
        <div className="pt-2">
          <Button
            onClick={async () => {
                const nets = [];
                if (netX) nets.push('x');
                if (!mealId || !whenISO || (nets.length===0 && selectedHooks.length===0)) return alert('Pick at least one channel');
                const r = await fetch('/api/chef/schedule/create', {
                  method: 'POST', headers: {'Content-Type':'application/json'},
                  body: JSON.stringify({ mealId, whenISO, networks: nets, webhooks: selectedHooks, caption, autoLastCall })
                });
              if (r.ok) { await loadSchedules(); alert('Scheduled'); }
              else {
      ...
      <div className="rounded-2xl border p-4">
        <h3 className="text-base font-semibold mb-2">Upcoming & recent posts</h3>
        {!schedules.length ? (
              ...
              <tbody>
                {schedules.map(s => (
                  <tr key={s.id} className="border-t align-top">
                    ...
                    <td className="py-2 pr-4 max-w-xl truncate" title={s.text}>{s.text}</td>
                  </tr>
                ))}
              </tbody>
            </table>
