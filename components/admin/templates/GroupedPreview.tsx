export function GroupedPreview({ branding, snapshotId, label, createdAt }: {
  branding: any;
  snapshotId: string;
  label: string;
  createdAt: string;
  ...
  return (
    <div>
      <div className="text-sm text-gray-100">Snapshot: <code>{snapshotId}</code></div>
      ...
      <div className="flex gap-2 mt-1 items-center">
        {branding?.primary_color && (
          <span className="inline-block w-4 h-4 rounded-full" style={{ backgroundColor: branding.primary_color }} title={`Primary: ${branding.primary_color}`}></span>
        )}
        {branding?.secondary_color && (
          <span className="inline-block w-4 h-4 rounded-full" style={{ backgroundColor: branding.secondary_color }} title={`Secondary: ${branding.secondary_color}`}></span>
        )}
        {branding?.id && (
