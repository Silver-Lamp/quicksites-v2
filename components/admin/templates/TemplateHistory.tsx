...
export default function TemplateHistory({ template, onRevert }: { template: any; onRevert: (data: any) => void }) {
          ...
          <li key={v.id} className="border p-3 rounded bg-white dark:bg-gray-900">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-xs text-gray-400">{new Date(v.created_at || v.saved_at).toLocaleString()}</p>
                {v.thumbnail_url && (
                  <img
                    src={v.thumbnail_url}
      ...
      {selectedVersion && (
        <div id="version-diff-container" className="mt-6">
          ...
          <div className="flex gap-4 mt-3">
            <Button size="sm" variant="secondary" onClick={handleRevert}>Revert to This</Button>
          </div>
        </div>
      )}
    </div>
