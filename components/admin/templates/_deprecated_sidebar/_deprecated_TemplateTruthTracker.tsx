...
function InfoDropdown({
  ...
  const loadDeprecated = async () => {
    try {
      const res = await fetch(`/api/templates/deprecations?id=${templateId}`, { cache: 'no-store' });
  ...
  const toggle = (f: string) => {
    const next = deprecated.includes(f) ? deprecated.filter((x) => x !== f) : [...deprecated, f];
    setDeprecated(next);
    ...
    <DropdownMenu open={isOpen} onOpenChange={handleOpenChange}>
      ...
      <DropdownMenuContent className="w-[380px] p-2">
        ...
        <div className="flex items-center justify-between px-2 pb-2 text-[11px]">
          ...
          <div className="flex items-center gap-2">
            <Button size="sm" variant="outline" className="h-7 px-2 gap-1" onClick={markAllShown}>
              <CheckSquare2 className="h-3.5 w-3.5" /> Mark all shown deprecated
            </Button>
            <Button size="sm" variant="outline" className="h-7 px-2 gap-1" onClick={unmarkAllShown}>
            ...
            {visibleFiles.map((f) => (
              ...
              >
                ...
                <label className="ml-2 inline-flex items-center gap-1 text-[11px]">
                  <input type="checkbox" checked={deprecated.includes(f)} onChange={() => toggle(f)} />
        ...
        <div className="flex items-center justify-between gap-2 px-2 pb-1">
          <Button size="sm" variant="outline" className="gap-1" onClick={copyFiles}>
        ...
        <div className="flex items-center justify-between gap-2 px-2 pb-2">
          ...
          <div className="ml-auto flex items-center gap-2">
            <Button size="sm" variant="outline" className="h-7 px-2 gap-1" onClick={copyScript} disabled={deprecated.length === 0}>
              <Clipboard className="h-3.5 w-3.5" /> Copy script
            </Button>
            <Button size="sm" variant="outline" className="h-7 px-2 gap-1" onClick={downloadScript} disabled={deprecated.length === 0}>
...
function Node({ title, icon, children, state }: { title: string; icon: React.ReactNode; children?: React.ReactNode; state?: 'ok' | 'warn' | 'empty' }) {
  return (
    <motion.div
...
function extractIndustry(evt: TemplateEvent): string | undefined {
  ...
  return (typeof m.industry === 'string' && m.industry) || (typeof m?.data?.meta?.industry === 'string' && m.data.meta.industry) || undefined;
}
  ...
  const out = asArr
    .map((item) => {
      ...
      if (item && typeof item === 'object') {
        ...
        const price = o.price != null && String(o.price).trim() !== '' ? ` — ${typeof o.price === 'number' ? `$${o.price.toFixed(2)}` : String(o.price)}` : '';
...
function extractServices(evt: TemplateEvent): string[] | undefined {
  const m = evt.meta as any;
  if (!m || typeof m !== 'object') return undefined;
  return normalizeServicesList(m.services) || normalizeServicesList(m?.data?.services) || undefined;
}
...
function TimelineItem({
  ...
  const blockDiff: BlockDiff | undefined = useMemo(() => {
    ...
    const before = (m.before?.data ?? m.dataBefore) ?? ((prevEvt?.meta as any)?.data ?? (prevEvt?.meta as any)?.snapshot?.data);
      ...
      <div className="flex w-full items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          <div className="mt-1 text-[11px] text-muted-foreground">
            ...
            <span>
              <span className="font-medium">services:</span> {servicesPreview.length > 0 ? servicesPreview.join(', ') : 'none'}
              {more > 0 ? ` … +${more} more` : ''}
            </span>
          </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {snapId && (
            <>
              <Button size="sm" variant="outline" className="h-7 px-2 gap-1" disabled={busy} onClick={doRestore} title="Restore draft to this point">
...
function BlockChangeChips({ diff }: { diff?: BlockDiff }) {
  ...
  const top = (obj: Record<string, number>, max = 4) => Object.entries(obj).sort((a, b) => b[1] - a[1]).slice(0, max);

  const addedTop = top(diff.addedByType);
  const modTop = top(diff.modifiedByType);
  const remTop = top(diff.removedByType);

  const none = addedTop.length === 0 && modTop.length === 0 && remTop.length === 0;
...
function DiffBadge({ added = 0, changed = 0, removed = 0 }: { added?: number; changed?: number; removed?: number }) {
  const hasAny = (added ?? 0) + (changed ?? 0) + (removed ?? 0) > 0;
  return (
    <div className={clsx('flex items-center gap-1 rounded-md border px-2 py-1 text-[11px]', !hasAny && 'opacity-60')}>
...
function DeprecatedBanner({ count, onReview }: { count: number; onReview: () => void }) {
  return (
    <div className="flex items-center justify-between rounded-lg border border-amber-300/50 bg-amber-50 px-3 py-2 text-xs text-amber-900">
      <div>
        <span className="font-medium">{count} file{count === 1 ? '' : 's'} marked deprecated.</span>
        <span className="ml-2 text-amber-800">Consider pruning once the new flow is stable.</span>
      </div>
...
function iconForEvent(t: TemplateEvent['type']) {
  switch (t) {
    case 'save':
      return { icon: <Save className="h-4 w-4" />, tone: 'blue', label: 'save' } as const;
    case 'autosave':
      return { icon: <HistoryIcon className="h-4 w-4" />, tone: 'gray', label: 'autosave' } as const;
    case 'snapshot':
...
export default function TemplateTruthTracker({
  ...
  const [fallbackState, setFallbackState] = useState<{ infra?: InfraState; snapshots?: SnapshotInfo[] } | null>(null);

  useEffect(() => {
    if (infra) return; // parent provided it
    let alive = true;
    (async () => {
      try {
        ...
        setFallbackState({
          infra: st?.infra,
          snapshots: Array.isArray(st?.snapshots) ? st.snapshots : undefined,
        });
      } catch {
  ...
  // NEW: central refresh that updates both infra and events
    ...
    try {
      ...
      setFallbackState((s) => {
        ...
        if (stRes.status === 'fulfilled') {
          next.infra = stRes.value?.infra ?? next.infra;
          next.snapshots = Array.isArray(stRes.value?.snapshots) ? stRes.value.snapshots : next.snapshots;
        }
        if (evRes.status === 'fulfilled') {
          const arr = Array.isArray(evRes.value) ? evRes.value : Array.isArray(evRes.value?.items) ? evRes.value.items : [];
          setLazyEvents(arr);
        }
        return next;
      });
  ...
  useEffect(() => {
    const handler = () => void refreshTruth();
    window.addEventListener('qs:truth:refresh', handler as any);
    return () => window.removeEventListener('qs:truth:refresh', handler as any);
  }, [refreshTruth]);
  ...
  const selectedVersionTag = useMemo(() => versions.find((v) => v.snapshotId === selectedSnapshot?.id), [versions, selectedSnapshot]);

  const [infoOpen, setInfoOpen] = useState(false);

  /* ---------------- timeline: lazy loader ----------------- */
  ...
  const effectiveEvents = useMemo(() => dedupeEvents((lazyEvents ?? events) ?? []), [lazyEvents, events]);
  /* -------------------------------------------------------- */

  // NEW: fallbacks for snapshot/publish/restore if parent didn't pass handlers
  ...
  const restoreFallback = useCallback(
    async (_id: string) => {
      // Wire this when your restore API exists
      safeTruthRefresh();
      await refreshTruth();
    },
    [refreshTruth],
  ...
  const doCreateSnapshot = onCreateSnapshot ?? createSnapshotFallback;
  const doPublish = onPublish ?? publishFallback;
  const doRestore = onRestore ?? restoreFallback;
    ...
    <TooltipProvider>
      <div className={clsx('flex h-full flex-col gap-3 p-2', className)}>
        <StateHeader
          ...
          onRefresh={onRefresh ?? refreshTruth}
          info={info}
        ...
        {deprecatedCount > 0 && <DeprecatedBanner count={deprecatedCount} onReview={() => setInfoOpen(true)} />}
                    ...
                    <ul className="relative mx-3 my-2">
                      ...
                      {effectiveEvents.length === 0 && (
                        <li className="px-3 py-2 text-xs text-muted-foreground">
                          {lazyEvents === null && (!events || events.length === 0) ? 'Open to load history…' : 'No events yet.'}
                        </li>
                      )}
        ...
        <CollapsiblePanel id="snapshot-picker" title="Snapshot Picker + Actions" defaultOpen={false} lazyMount>
          <Card className="border-neutral-200">
            ...
            <CardContent className="flex flex-col gap-2">
              ...
              <div className="flex items-center gap-2">
                <Button size="sm" variant="default" onClick={doCreateSnapshot} className="gap-1">
                ...
                <Button
                  ...
                  className="gap-1"
                  onClick={() => selectedSnapshot?.id && doPublish(selectedSnapshot.id)}
                >
