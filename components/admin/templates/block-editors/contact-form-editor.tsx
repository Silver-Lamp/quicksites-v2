...
export function ContactFormEditor({ block, onSave, onClose, template }: BlockEditorProps) {
  ...
  // Optional subset stored in block content
  const [services, setServices] = useState<string[]>(
    Array.isArray((formBlock.content as any)?.services) ? ((formBlock.content as any).services as string[]) : []
  );

  // ✅ Source of truth for options is the DB column
  const allAvailableServices: string[] = Array.isArray(template?.services) ? (template.services as string[]) : [];
  ...
  return (
    <div className="p-5 space-y-5 rounded-xl border bg-white text-zinc-900 shadow-sm
      ...
      <div className="space-y-1.5">
        <label className="block text-sm font-medium text-zinc-800 dark:text-zinc-200">Form Title</label>
        <input
          type="text"
          value={title}
        ...
        <div className="relative">
          ...
          <input
            ...
            className={`w-full rounded-md border bg-white px-9 py-2 text-sm
                        ...
                        ${emailOk ? 'border-zinc-300 focus:ring-purple-500 dark:border-zinc-800' : 'border-red-500/70 focus:ring-red-500/70'}`}
          />
        </div>
      ...
      <div className="space-y-2">
        <label className="block text-sm font-medium text-zinc-800 dark:text-zinc-200">Included Services</label>
        ...
        {allAvailableServices.length > 0 ? (
              ...
              return (
                <label
                  ...
                  className={`flex cursor-pointer items-center gap-2 rounded-md border px-3 py-2 text-sm
                              ${checked ? 'border-purple-500/60 bg-purple-500/10' : 'border-zinc-300 hover:bg-zinc-50 dark:border-zinc-800 dark:hover:bg-zinc-950'}`}
                >
                  <input
                    type="checkbox"
          ...
          <div className="rounded-md border border-dashed border-zinc-300 px-3 py-3 text-sm text-zinc-500
            ...
            No services defined in this template. Update services in the sidebar first.
          </div>
        )}
      </div>
      ...
      <div className="flex justify-end gap-2 pt-2">
        ...
        <button
          onClick={() => {
            ...
            onSave({
              ...formBlock,
              // ✅ Save only title/email + the sanitized subset; options always come from DB
              content: { title, notification_email, services: sanitizedSubset },
            });
          }}
          disabled={!emailOk}
