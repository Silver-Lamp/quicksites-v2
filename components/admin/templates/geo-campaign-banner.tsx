...
export default function GeoCampaignBanner({ c }: { c: GeoCampaignSummary }) {
  ...
  const topRec = plan
    ...
    : Array.isArray(c.recommendations?.ranking)
      ...
      : null;
  return (
    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 border-b border-fuchsia-500/40 bg-fuchsia-950 px-4 py-2 text-sm text-fuchsia-50">
      <span className="inline-flex items-center gap-1.5 font-medium">
        <MapPin className="h-4 w-4" /> Geo-domain campaign
      </span>
      <span className="font-mono text-fuchsia-200">{c.domain}</span>
      {c.city ? <span className="text-fuchsia-200">{c.city}</span> : null}
      ...
      {c.tracking_number && <span className="text-xs text-fuchsia-200">☎ {c.tracking_number}</span>}
      {nextAction && (
      ...
      {topRec && <span className="hidden text-xs text-fuchsia-200 md:inline" title={topRec.detail}>💡 {topRec.title}</span>}
      <Link href="/admin/prospects" className="ml-auto text-xs text-fuchsia-100 underline underline-offset-2 hover:text-white">
        Manage in Prospects →
      </Link>
    </div>
  );
