// components/admin/templates/render-blocks/auto-shops-directory.tsx
'use client';

// The apex-portal directory block (auto-shop domain-competitions): renders the contest
// cohort as driver-facing shop cards, WINNER featured first — the SecondSet transparency
// wedge ("shops that show you the work"). Content carries a `campaign_id` (drives a live
// fetch so winner changes show without a republish) + an `entries` snapshot (instant paint
// / fallback when the API is unreachable). Same renderer in the editor and on the site.
...
function Card({ entry, featured }: { entry: Entry; featured?: boolean }) {
  return (
    <a
      ...
      className={`group block overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900/50 transition hover:border-emerald-500/50 hover:bg-zinc-900 ${
    ...
    >
      <div className={`relative w-full overflow-hidden bg-zinc-800 ${featured ? 'aspect-[2/1]' : 'aspect-[3/2]'}`}>
        ...
        {entry.is_winner && (
          <span className="absolute left-3 top-3 rounded-full bg-emerald-500 px-2.5 py-1 text-xs font-semibold text-zinc-950 shadow">
            ★ Featured
          </span>
        )}
      </div>
      <div className="p-4">
        <div className="text-base font-semibold text-white group-hover:text-emerald-300">{entry.business_name}</div>
        <div className="mt-1 text-xs text-zinc-500">See the work before you pay →</div>
      </div>
    </a>
  );
}

export default function AutoShopsDirectoryBlock({
  block,
  content,
  previewOnly,
}: {
  ...
  React.useEffect(() => {
    ...
    let alive = true;
    fetch(`/api/public/auto-shop-directory?campaign=${encodeURIComponent(campaignId)}`, { cache: 'no-store' })
      ...
      <section className="mx-auto w-full max-w-5xl px-6 py-10">
        ...
        <div className="rounded-2xl border border-dashed border-zinc-700/70 px-6 py-14 text-center text-sm text-zinc-500">
          {previewOnly || campaignId
            ? 'Auto shops appear here automatically — the live contest cohort, winner featured first.'
            : 'Link this block to an auto-shop competition (campaign_id) to list its cohort here.'}
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="mx-auto w-full max-w-5xl px-6 py-10">
      ...
      <div className="mt-10 border-t border-zinc-800/60 pt-4 text-center text-xs text-zinc-500">
        Shops that show you the work — a photo + the tech&apos;s note, so you approve before they fix · powered by {brand}
      </div>
    </section>
  );
}
