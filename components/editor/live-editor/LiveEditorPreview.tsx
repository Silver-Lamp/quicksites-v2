...
type RenderSite = {
  ...
  data: any; // keep original for downstream expectations
};
...
function normalizeForRenderer(snapshotData: any, siteFields: Pick<SiteRow, 'id'|'slug'|'domain'|'default_subdomain'>): RenderSite {
  ...
  const headerBlock =
    data?.headerBlock ?? data?.header ?? null;
  const footerBlock =
    data?.footerBlock ?? data?.footer ?? null;
...
async function loadDraftTemplate(templateId: string): Promise<any | null> {
  const { data, error } = await supabaseAdmin
    .from('templates')
  ...
  // map draft into snapshot-like data for normalization
  const d = (typeof data?.data === 'string') ? safeParse(data?.data) : (data?.data ?? {});
...
async function loadSiteForRequest(previewSnapshotId?: string | null): Promise<{ site: RenderSite; host: string } | null> {
  ...
  // Backstop: no snapshot — try the draft template as last resort (dev)
...
export default async function HostSitePage({
  ...
  qlog('render pages', { count: site.pages?.length ?? 0, slugs: (site.pages ?? []).map((p: any) => p.slug) });
