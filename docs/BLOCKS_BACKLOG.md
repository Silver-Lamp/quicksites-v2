...
| 4 | **Audio FAQ** | UNBLOCKED — builds when contract flips LIVE | HJ shipped the `faq_answer` register (PR #1310): `POST /api/about-that/embed/:id/ask {url,question} → {answer, answerable, audio_url}` — grounded strictly in the page's text (off-page → graceful decline), owner-billed, domain-gated, cached. Maps 1:1 to a per-page "ask + hear the answer in the owner's voice" block. QS build waits only on the endpoint landing in `crosstalk/contracts/about-that-embed.md` as LIVE (requested 2026-07-17), then ships same-day (gates on valid embed id; declines → "contact the site"). |
