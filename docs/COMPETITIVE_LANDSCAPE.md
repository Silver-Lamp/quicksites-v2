...
2. **Platform-revenue reconciliation surface** *(M)* — sum Stripe `application_fee` ↔ `orders.platform_fee_cents`, show "QS earned $X / partners owed $Y." Already partly built (`app/api/admin/commerce/reconcile`); finish the dashboard number.
3. **Consolidate Connect onboarding on `payment_accounts`** *(S)* — delete the deprecated `merchant_payment_accounts` path so the money story is single-brained.
...
9. **Tax/shipping basics** *(M)* — automated tax + flat/zoned shipping for physical/POD orders, so author/apparel merchants aren't blocked.

### Tier 3 — Lean into what neither competitor has
10. **POD/author funnel as a flagship demo** *(M)* — GHL has zero ecommerce and Duda has no print-on-demand; the Lulu/Gelato author site is a category neither can answer. Make it the showcase. (See [`POD_AUTHOR_PLAN.md`](POD_AUTHOR_PLAN.md).)
11. **GMV-based pricing calculator** *(S)* — a "Duda charges you $149 flat; here you earn 80% of every sale" comparison page targeting agencies. Weaponize the §7 positioning.
