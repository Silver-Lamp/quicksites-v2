...
4. ~~**Partner self-serve onboarding** *(L)*~~ ✅ — the no-admin loop is complete: `POST /api/partners/join` self-issues a `referral_code` (idempotent), the one-click JoinButton on `/partners/dashboard` enrolls any signed-in user, the shareable `/join/<code>` invite is **white-labeled per reseller org** (logo/accent/brand via `resolveOrg`), and payouts self-connect via `/api/partners/connect/onboard` — no admin touch anywhere. The surfaces were built but the loop **didn't pay**: `ensureAttributionForMerchant` wrote `attributions` (RLS deny-default: RLS on, no policies) with a user-scoped client, so every self-serve attribution was silently RLS-rejected → merchant bound to nobody → `markOrderPaid` found no `referral_code` → **$0 residual**. The green-path demos masked it by inserting `attributions` with the service role directly. Fixed to use the service-role client (matching `markOrderPaid`'s read) + surface (not swallow) write errors; regression-tested (`lib/commerce/__tests__/attribution.test.ts`, 6 cases) pinning the service-role contract + lock/insert/update/no-ref behaviors.
...
6. ~~**Partner-facing residual dashboard** *(M)*~~ ✅ — `/partners/dashboard` shows a partner's live "lifetime earned / pending payout / paid out," referred-merchant count, per-merchant earnings, and payout history (riding `commission_ledger` + `attributions` + `affiliate_payouts` via `getPartnerStats`), linked from `/partners` + the calculator. The remaining gap was a correctness one exposed by #12: the "pending payout" figure counted only `pending`, so a commission the newly-working auto-approval cron moved `pending → approved` would **disappear** from the partner's visible balance before being paid. Fixed by surfacing **owed = pending + approved** as the headline (with a breakdown line) and an **Unpaid** per-merchant column; the aggregation is now a pure, unit-tested `aggregatePartnerLedger()` (`lib/commerce/partnerStats.ts`, 7 cases).

### Tier 2 — Match table-stakes storefront depth (where Duda is ahead)
7. ~~**Storefront hardening** *(M–L)*~~ ✅ (core) — the credible vertical is the **generic `catalog_items` storefront** (`/store/[merchant]` → `/p/[slug]` → cart → `/checkout` → `POST /api/commerce/checkout` → order → platform fee → commission), since the food/meals vertical was retired (`drop_meals`). The blocker was a **critical price-tampering hole**: the checkout trusted the client's `unitAmount`, so a buyer could POST `unitAmount:1` and buy anything for a penny (and set their own item title into Stripe). Fixed with **server-side price authority** — `authorizeCheckoutItems()` (`lib/commerce/checkoutItems.ts`) reprices every line from `catalog_items.price_cents`, enforces item∈merchant + `status='active'` + a valid (non-null) price + a sane quantity cap, and rejects the rest; the client `unitAmount`/`title` are now ignored. Unit-tested (`checkoutItems.test.ts`, 9 cases incl. the tamper, cross-store, inactive, unpriced, and multi-line paths). Product **variant** SKUs (size/color) also shipped: variants live on `catalog_items.metadata.variants` (`[{id,label,price_cents,status}]`, no migration), authored in the catalog drawer, selected on `/p/[slug]`, carried through the cart (composite line id so red-M ≠ blue-L) and checkout — and, crucially, **repriced server-side per variant** by the same `authorizeCheckoutItems()` (item-with-variants requires a valid active variant; the client-sent price is ignored). We don't chase Duda's 20k-product depth — but a merchant can now sell one product in several priced options that visibly + safely works. **Multi-axis variants** (Size × Color grids) are supported too: `metadata.variant_options` holds the axes and each offered combination is a flat SKU in `metadata.variants` (so checkout stays axis-blind — `authorizeCheckoutItems` still resolves by SKU id). The storefront renders one selector per axis and resolves the combination → SKU (`resolveVariantByOptions`, unit-tested), disabling unoffered combos; the shared `VariantsEditor` authors the axes + generates the priced grid, normalized server-side by `normalizeVariants()` (`lib/commerce/variants.ts`, unit-tested). Existing items are **editable** too — `GET`/`PATCH /api/catalog/items/[id]` + an `EditItemDrawer` (reusing `VariantsEditor` seeded from current metadata) let a merchant change title/price/status and add, reprice, or clear variants; the metadata merge (which correctly clears stale `variant_options` when variants are removed) is the unit-tested `mergeVariantMetadata()`. **Per-SKU inventory** is supported too (`lib/commerce/inventory.ts`): a plain item (`metadata.stock`) or any variant can carry a unit count (blank = untracked/unlimited); `authorizeCheckoutItems` refuses to oversell (sold-out / "only N left"), the storefront disables sold-out selections + shows a low-stock nudge, and `markOrderPaid` decrements the ordered SKU once per paid order via an **atomic, row-locked** Postgres function (`decrement_catalog_stock`, `20260702_atomic_stock_decrement.sql`, server-role-only) so concurrent orders serialize and the DB refuses to oversell past what exists (the second racer for a last unit is rejected, not silently clamped) — read-side gate + normalization stay pure/tested (`checkStock`, `normalizeStock`). **Reserve-then-confirm** goes further: the commerce checkout now *reserves* stock at draft time (atomic decrement + a `stock_reservations` hold), so a buyer who loses a stock race is told **"just sold out" before paying** (409, draft cancelled) rather than being refunded after. Payment consumes the hold; a genuine Stripe failure or an abandoned checkout releases it (units returned) — abandoned holds are swept by `/api/cron/release-stock-reservations` (every 15 min) past their 30-min TTL. `markOrderPaid` consumes reservations when present and otherwise falls back to the atomic decrement (for demo/non-reserved paths), whose leftover oversell — should one ever occur — is still recorded to `orders.oversold_lines` and **surfaced to the merchant** (`/merchant/orders` amber banner + per-order breakdown). Reservation fns are server-role-only; the whole reserve→consume/release/sweep lifecycle is verified against the live DB. **Per-variant images** round it out: each SKU can carry its own image URL (authored per combo in `VariantsEditor`), and the product page (`ProductDetail` client wrapper) swaps the displayed image when a variant with its own image is selected, falling back to the item's main image. Images can be **uploaded** (not just pasted as URLs): a compact `ImageUploadField` follows the established browser-direct-upload pattern (session-aware client → public `images` bucket → public URL), wired into the item main image (create + edit) and each variant in `VariantsEditor` — this is also the first in-UI way to give a catalog item a main image.
...
12. ~~**Automated commission approval rule** *(M)*~~ ✅ — `pending → approved` after the refund window, so payouts don't bottleneck on manual review. The rule (`approveCommissions` in `lib/commerce/payouts.ts`), the cron (`app/api/cron/approve-commissions`, daily 08:00), and the manual admin trigger were all present — but the cron **only** checked the custom `x-cron-secret` header, which **Vercel's scheduler never sends** (it uses `Authorization: Bearer <CRON_SECRET>`), so the registered job was silently 403'd and the loop never actually ran. Fixed by switching to the canonical `isCronAuthorized` (like every sibling cron); added a regression test (`lib/cron/__tests__/auth.test.ts`) pinning the Bearer + custom-header contract.
