...
- **Organize & engage (Phase 2)** — segments/filters/sort/tags (`lib/crm/segments.ts`), editable notes/tags/`marketing_consent` (`CustomerAdminPanel` → owner-gated `PATCH /api/merchant/customers/[id]`, `customers.notes` col via `20260707_customers_notes.sql`), a unified **activity timeline** (`lib/crm/activity.ts` — orders + campaign receipts, keyed to `customer_id`), and **dedup/merge** — since the spine keys on `email_normalized`, duplicates are the same person under different emails, detected by shared phone/name (`lib/crm/dedup.ts`) and folded via the owner-gated `POST /api/merchant/customers/merge` → `merge_customers` RPC (`20260708_merge_customers.sql`: repoints orders + campaign sends, sums rollups, unions tags, concatenates notes, keeps the survivor's consent, deletes losers). A "possible duplicates" panel on the list (`components/merchant/CustomerDedupPanel.tsx`) lets the owner pick the survivor.
...
- **Enrichment follow-ups** (not blocking): ~~buyer/campaign PostHog events~~ **DONE** — `customer_created`/`repeat_purchase`/`campaign_sent`/`campaign_order_attributed`/`customer_unsubscribed` emit via `captureServer` (`lib/analytics/events.ts`, distinctId = the customer). campaign **open/click** metrics — endpoint built (`POST /api/crm/webhooks/resend`, Svix-verified; stamps `opened_at`/`clicked_at`/`complained_at` on `crm_campaign_sends`, shown in the history table), pending Resend dashboard webhook config + `RESEND_WEBHOOK_SECRET`. Still open: **SMS** campaigns (`sendSms` rails exist); an **outbox/drain** path for sends above the 250 cap. ~~dedup/merge UI~~ **DONE** (see Phase 2 above).
...
3. **Dedup + merge** — ✅ same-person-different-email duplicates surfaced by shared phone/name (`lib/crm/dedup.ts`), folded via `merge_customers` RPC + a "possible duplicates" panel on the list.
