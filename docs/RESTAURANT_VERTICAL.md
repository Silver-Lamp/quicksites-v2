...
- **Batch engine**: `npm run import:listings -- leads.json` (`scripts/import-listings-batch.ts`) turns a list of leads (a `query` like "Hawkers Bar & Grill, Auburn WA", a `placeId`, or a pasted `listing`, each with optional menu `photoUrls`) into claimable drafts, and writes `<leads>-results.json` with each draft's **preview URL** (`/preview/<slug>`) for outreach. `findPlace()` resolves a name → Place ID (Text Search). Owner via `CEDARSITES_OPERATOR_ID` (optional).
