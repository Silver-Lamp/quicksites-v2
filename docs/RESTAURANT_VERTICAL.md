...
- **Full-funnel demand proof** (admin-gated): `POST /api/admin/commerce/demand-demo` — the whole no-website funnel (§7c) end-to-end: seed a `listing_import` draft (menu block + listing phone) → `recordDemandEvent` ×3 → assert the claim-pitch count/leads → `resolveListingPhone` (the Phase-2 SMS precondition) → `claim_operator_draft` transfers ownership + flips `claim_source` → `getDemandDetails` still returns the leads (`/welcome` payoff) → `resolveMerchantFeeDefault` → **8% + 60¢** → two orders assert **$5 → 60¢ (floor bites)** and **$30 → 240¢ (percent bites)**. `{cleanup:true}` tears down.
...
- **One canonical host per city apex (2026-09-14).** A `<city>-restaurant` apex answers on up to three hosts — `<city>-restaurant.com`, `www.`, and `<city>-restaurant.delivered.menu` — and every render is self-canonical by design (`lib/seo/canonicalUrl.ts`), so search engines saw three copies of one page (Renton, Kent and Paterson all did). `data.meta.canonical_origin` nominates the one host the others defer to; the menu-host render swaps only the origin and keeps the requested path. It is written by **`node scripts/set-apex-canonical.mjs <slug> <https-origin>`**, which refuses unless the nominated origin answers 200 right now with the same `<title>` — a campaign row's `domain_status` is not evidence (three of them read `planned`/`failed` for domains that served), an HTTP response is. The script also corrects such a row to `attached`.
- **Launching an apex for a batch-imported city.** `scripts/import-listings-batch.ts` writes templates and no `outreach_prospects` rows, so its cities (Elizabeth, Naples, Union City, …) were invisible to the competition machinery. **`npx tsx scripts/launch-batch-city-apexes.ts --city <City> --state <ST> [--dry-run]`** backfills a prospect per unclaimed draft from the listing's own facts (`source_place_id`, name, phone, address — nothing inferred), launches through the same `createRestaurantCompetition` the admin button calls, and **hides placeholder-menu drafts from the public directory** (`directoryCuration`) so a diner is never linked to a menu nobody wrote (#738). It refuses to run with `GEO_DOMAIN_REGISTER_ENABLED` set: the apex lives on the delivered.menu subdomain and the `.com` is a separate purchase.
...
**Feeding demand (the traffic lever):** a pre-claim draft is `noindex` and isn't in the apex directory, so on its own it has **no traffic → no demand**. Two sources: (1) `MENU_DRAFT_INDEXABLE` (flag, OFF) lets a `listing_import` menu draft on the menu host be **indexed** — for a no-website restaurant its delivered.menu page *is* their web presence, so ranking for their name feeds demand (watermark stays); admin previews + every other draft stay `noindex` regardless. (2) A diner-facing **order QR** (encodes the public menu URL, distinct from the owner *claim* QR): the batch importer writes `<slug>-order.png` next to `<slug>.png`, and `/admin/outreach` has a per-draft "Order QR ⤓" (`GET /api/admin/outreach/[id]/order-qrcode`) to print + place at the restaurant (window sticker / table tent).
...
- `MENU_DRAFT_INDEXABLE` — when on, a `listing_import` menu draft on the menu host is **indexable** (the no-website restaurant's search presence, feeding demand); watermark unchanged. Off by default — flip at go-live once `delivered.menu` DNS is pointed. See §7c.
