...
| 3b. **Measure the response** | `/go/<prospectId>` counts `outreach_prospects.claim_link_visits` and mirrors `trade_card_link_visited` to PostHog; `lib/outreach/cardResponse.ts` (pure) → the **"Claim cards → scans → claims"** section on `/admin/ops` (mailed · should-have-arrived · delivered · QR scanned · claimed, by metro) | on scan / on dashboard load | **automated (2026-09-16)** — before this, 72 cards were in the mail and the only way to see a scan was SQL. ⚠️ "arrived" is Lob's forecast; "delivered" reads 0 until the owner registers the Lob webhook; a scan dated before the forecast is counted as a test scan, not a response |
