...
⚠️ **THIS LIST SHRANK. It was four questions; three were answered by §3a, and this section kept
asking them anyway.** Left as-is, a reader who skims to "open questions" — which is what this
section is *for* — gets the pre-correction version of the plan and re-decides a fork that had
already dissolved. Same failure as a stale count in a memory file: the correction lands in one
place and the superseded version is left standing somewhere it reads as current. Cheaper to catch
in a doc than in a mailing.
...
2. **September, or spring — as an instrument choice, not a season.** §3a dissolved the *seasonality*
   ...
   So the question is not "which season" but **"the clean instrument, or the early read."**

**Closed, and by what — kept rather than deleted, so the reasoning stays checkable:**
...
| The neighbour's own sale code, or generic? | **Theirs.** The artifact *is* their page; a generic promo card is us using them to market. | §3a; HJ concurred independently |
| Is Step 2 a green light for Step 3? | **No — a kill gate only.** A warm pass cannot authorize cold postage. | §4 Step 2 (both reviewers found this hole) |
