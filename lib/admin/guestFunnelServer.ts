...
import { computeGuestFunnel, EMPTY_GUEST_FUNNEL, type GuestFunnel, type GuestTemplateRow, type GuestUserRow } from './guestFunnel';
import { contactOnRecord } from './guestContacts';

export async function loadGuestFunnel(): Promise<GuestFunnel> {
  try {
    const admin = supabaseAdmin as any;
    const [{ data: lu }, { data: tpls }] = await Promise.all([
    ...
    return computeGuestFunnel(guests, templates);
  } catch (e) {
