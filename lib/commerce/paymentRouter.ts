...
export async function getMerchantPaymentConfig(merchantId: string): Promise<PaymentConfig> {
  ...
  const { data, error } = await supabase
    .from('payment_accounts')
    .select(
      'provider, account_ref, collect_platform_fee, platform_fee_percent, platform_fee_min_cents, status'
    )
    .eq('merchant_id', merchantId)
    .eq('status', 'active')
    .limit(1)
...
export async function createCheckout(
  merchantId: string,
  p: Omit<CreateCheckoutParams, 'platformFeeCents' | 'connectAccountId'>
) {
