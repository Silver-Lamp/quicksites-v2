...
export async function fulfillOrderPodItems(
  orderId: string,
  eventRaw?: any
  ...
  try {
    ...
    const { data: order } = await (supa as any)
      .from('orders')
      .select('id, merchant_id, currency, raw')
      .eq('id', orderId)
      .single();
    ...
    const { data: items } = await (supa as any)
      .from('order_items')
      .select('id, catalog_item_id, title, quantity, metadata')
      .eq('order_id', orderId);
    ...
    for (const it of items) {
      ...
      if (!ship?.line1 || !isProviderConfigured(provider)) {
        await recordPrintOrder({
          ...base,
          status: !ship?.line1 ? 'awaiting_address' : 'awaiting_fulfillment',
        });
        queued++;
        continue;
      }
      ...
      try {
        if (provider === 'lulu') {
          const job = await lulu.createPrintJob({
            ...
            pageCount: Number(spec.pageCount) || 0,
            ...
            shippingAddress: {
              ...
              postcode: ship.postal || '',
              country_code: ship.country || 'US',
              phone_number: ship.phone || '0000000000',
              email: ship.email,
            },
          ...
          const order2 = await gelato.createGelatoOrder({
            orderReferenceId: `${orderId}:${it.id}`,
            customerReferenceId: ship.email || orderId,
            productUid: spec.productUid,
            fileUrl: spec.fileUrl,
            quantity: Math.max(1, Number(it.quantity) || 1),
            currency: order.currency || 'USD',
            shippingAddress: {
              firstName: (ship.name || 'Customer').split(' ')[0],
          ...
          await recordPrintOrder({
            ...base,
            provider_job_id: String(order2.id),
            status: order2.fulfillmentStatus || 'created',
          });
        }
        created++;
      } catch (e: any) {
        await recordPrintOrder({
          ...base,
          status: 'error',
          raw: { ...base.raw, error: String(e?.message || e).slice(0, 300) },
        });
        queued++;
      }
    }
