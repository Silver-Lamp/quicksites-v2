...
export default function AccessRequestDashboard() {
  ...
  return (
    <>
      <Head><title>Access Requests</title></Head>
        ...
        <ul className="space-y-4">
          {requests.map((r) => (
            <li key={r.id} className="bg-white rounded shadow p-4">
              ...
              <p><strong>Status:</strong> <span className="font-semibold">{r.status}</span></p>
              <p className="text-sm text-gray-500 whitespace-pre-wrap mt-2">{r.message}</p>
              <div className="mt-2 flex gap-2">
                <button onClick={() => updateStatus(r.id, 'approved')} className="text-sm bg-green-600 text-white px-3 py-1 rounded">Approve</button>
                <button onClick={() => updateStatus(r.id, 'rejected')} className="text-sm bg-red-600 text-white px-3 py-1 rounded">Reject</button>
              </div>
            </li>
          ))}
        </ul>
