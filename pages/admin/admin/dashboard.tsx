...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const fetchCounts = async () => {
      ...
      const [{ count: viewsCount }, { count: linksCount }, { count: emailsCount }, { count: logsCount }] = await Promise.all([
        ...
        {hasRole(['admin', 'owner']) && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card className="relative border-l-4 border-blue-500 transition-transform duration-200 hover:scale-[1.01] hover:shadow-md">
              <div className={`absolute top-2 right-3 text-sm font-mono ${dailyViews > 100 ? 'text-green-500 animate-pulse' : 'text-blue-600'}`}>
  +{embedViews} views
  <br />
  ({dailyViews} today)
</div>
              ...
              <CardContent>
                ...
                <a href="/admin/embed-views" className="text-blue-600 text-sm underline">View Logs →</a>
                <button
                  onClick={async () => {
            ...
            <Card className="relative border-l-4 border-green-500 transition-transform duration-200 hover:scale-[1.01] hover:shadow-md">
              <div className={`absolute top-2 right-3 text-sm font-mono ${links > 50 ? 'text-green-600 animate-pulse' : 'text-green-600'}`}>
                {links} links
              </div>
              ...
              <CardContent>
                ...
                <a href="/admin/links" className="text-blue-600 text-sm underline">Open Shortlinks →</a>
                <button
                  onClick={async () => {
                    const token = localStorage.getItem('sb-access-token');
            ...
            <div className={`absolute top-2 right-3 text-sm font-mono ${emails > 10 ? 'text-yellow-600 animate-pulse' : 'text-yellow-600'}`}>{emails} emails</div>
            <CardHeader className="text-lg font-semibold text-yellow-600">📬 Email Summary</CardHeader>
              <CardContent>
                <p className="text-sm text-gray-500 mb-2">Review auto-generated summaries or drafts.</p>
                <a href="/admin/email-summary" className="text-blue-600 text-sm underline">Check Inbox →</a>
              </CardContent>
            </Card>
            <Card className="relative border-l-4 border-purple-500 transition-transform duration-200 hover:scale-[1.01] hover:shadow-md">
            <div className={`absolute top-2 right-3 text-sm font-mono ${logs > 25 ? 'text-purple-600 animate-pulse' : 'text-purple-600'}`}>{logs} logs</div>
            <CardHeader className="text-lg font-semibold text-purple-600">📥 Export Logs</CardHeader>
              <CardContent>
                <p className="text-sm text-gray-500 mb-2">Download platform usage logs and events.</p>
                <button
                  onClick={async () => {
                    const token = localStorage.getItem('sb-access-token');
