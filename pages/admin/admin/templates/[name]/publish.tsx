...
export default function PublishTemplate({ initialDomain, template_name }: { initialDomain: string, template_name: string }) {
  ...
  return (
    <div className="max-w-xl mx-auto p-6">
      ...
      <Input
        ...
        className="mb-4"
      />
      <Button onClick={handlePublish} disabled={!domain}>Publish</Button>
      {domain && (
        <div className="mt-4">
      ...
      {status === 'success' && <p className="text-green-600 text-sm mt-2">✅ Template published to {domain}</p>}
      {status === 'error' && <p className="text-red-600 text-sm mt-2">❌ Failed to publish template</p>}
    </div>
  );
}
