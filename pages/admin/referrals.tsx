...
export default function ReferralsPage() {
  ...
  const referredMap = Object.fromEntries(users.map(u => [u.id, u.user_metadata?.referrer_id || null]));

  const downstreamCount = (refId: string) => users.filter(u => u.user_metadata?.referrer_id === refId).length;
      ...
      <table className="w-full text-sm text-left text-gray-300 bg-gray-800 rounded overflow-hidden">
        ...
        <tbody>
          {referrers.map((r, i) => {
            ...
            const estimatedEarnings = estimatedSites * 49 * 0.10;
