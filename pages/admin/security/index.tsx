  ...
  return (
    <div className="p-6 text-text">
      ...
      <div className="space-y-3">
        <p>Last Slack Alert: <span className="text-yellow-400">{lastAlert || 'None recorded'}</span></p>
        <p>Expired tokens cleaned: <span className="text-green-400">{cleanupCount}</span></p>
      </div>
      <div className="mt-6 bg-zinc-900 p-4 rounded text-sm">
        <h2 className="font-semibold text-white mb-2">🔍 Log Preview</h2>
        <pre className="whitespace-pre-wrap text-zinc-400 max-h-[60vh] overflow-auto">{summary}</pre>
      </div>
    </div>
  );
}
