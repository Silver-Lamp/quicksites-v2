        ...
        {campaigns.map((c, i) => (
          <li key={c.slug} className="bg-zinc-800 rounded p-4 flex justify-between items-center">
            <div className="flex items-center gap-3">
              {i < 3 && (
                <img src={`/api/badge/${c.slug}`} alt="badge" className="w-12 h-12 rounded border border-zinc-700" />
              )}
              <div>
                <Link href={`/support/${c.slug}`} className="text-blue-400 font-semibold hover:underline">
                  {c.headline}
                </Link>
