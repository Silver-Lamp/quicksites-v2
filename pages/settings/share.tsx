...
export default function ShareSettings() {
  ...
  return (
    <div className="p-6 text-white max-w-xl mx-auto space-y-4">
      ...
      <p className="text-zinc-400 text-sm">This page controls what others can see at /world/[handle]/share</p>

      <div>
        <label className="block text-sm mb-1">Emoji</label>
        <input value={emoji} onChange={(e) => setEmoji(e.target.value)} className="text-black w-full p-1 rounded" />
      </div>

      <div>
        <label className="block text-sm mb-1">Short Bio</label>
        <textarea value={bio} onChange={(e) => setBio(e.target.value)} rows={2} className="text-black w-full p-1 rounded" />
      </div>

      <div>
        <label className="block text-sm mb-1">Goal Tags (comma separated)</label>
        <input value={tags} onChange={(e) => setTags(e.target.value)} className="text-black w-full p-1 rounded" />
      </div>
