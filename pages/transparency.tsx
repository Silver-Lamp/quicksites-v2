      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {sites.map((site: any) => (
          <div key={site.domain} className="bg-zinc-800 rounded shadow p-4">
            <img src={`/screenshots/${site.domain}.thumb.png`} alt={site.domain} className="w-full h-32 object-cover rounded mb-2" />
            <h2 className="text-lg font-semibold">{site.domain}</h2>
            <p className="text-sm text-zinc-400">{site.template_id}</p>
            <p className="text-xs text-zinc-500">{new Date(site.claimed_at).toLocaleDateString()}</p>
          </div>
        ))}
      </div>
    </div>
