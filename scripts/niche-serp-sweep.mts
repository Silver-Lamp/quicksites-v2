...
//
// ⚠️ COSTS MONEY: one call per (niche × query × city). It prints the count and the estimate
...
async function main() {
  ...
  const calls = candidates.reduce((s, c) => s + c.queries.length, 0) * cities.length;
  console.log(
    `${candidates.length} niches x ${cities.length} cities = ${calls} checks, about $${(calls * PER_CALL_USD).toFixed(2)}.` +
      (APPLY ? '' : ' Dry run — nothing fetched, nothing charged. Add --apply.'),
  );
  if (!APPLY) return;

  type Row = { key: string; label: string; green: number; total: number; failed: number; notes: string[] };
  const rows: Row[] = [];
      ...
      for (const q of c.queries) {
        ...
        try {
          ...
          const { data: prior } = await supabaseAdmin
            ...
            .select('verdict')
            .eq('niche_key', c.key).eq('query', r.query).eq('location', r.location)
          ...
          await supabaseAdmin.from('serp_observations').insert({
            niche_key: c.key, query: r.query, location: r.location, provider: 'dataforseo',
          ...
          process.stdout.write(`  ${c.key} ${city}: ${r.verdict} (pack ${r.packSize})            \r`);
        } catch (e) {
          row.failed++;
        }
      }
