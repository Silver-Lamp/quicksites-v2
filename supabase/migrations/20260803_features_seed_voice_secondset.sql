...
insert into public.features (id, org_id, title, blurb, category, doc_href, badge, featured)
select
  v.id,
  (select org_id from public.features group by org_id order by count(*) desc, org_id limit 1),
  v.title, v.blurb, v.category, v.doc_href, v.badge, v.featured
from (
  values
    (
...
) as v(id, title, blurb, category, doc_href, badge, featured)
